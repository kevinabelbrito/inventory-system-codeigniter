...
<div class="modal fade" id="ModalEditar<?= $id  ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<?= form_open('variaciones/editar/'.$id, array('id' => 'categoriasFormEditar'.$id)) ?>
			...
			<div class="modal-body bg-info">
				...
				<div class="row">
					...
					<div class="col-xs-6">
						<?= form_input('slug', $slug, array('id' => 'descripcion', 'maxlength' => 50)) ?>
					</div>
				</div>
			</div>
...
<script>
	$(function(){
		...
		$("#categoriasFormEditar<?= $id ?>").submit(function(){
			...
			$.post("<?= base_url() ?>variaciones/editar/<?= $id ?>", datosForm, procesar).error(procesarError);
			return false;
		});
